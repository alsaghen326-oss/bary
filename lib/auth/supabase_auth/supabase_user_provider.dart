...
class SalesManagerAppSupabaseUser extends BaseAuthUser {
  SalesManagerAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> salesManagerAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SalesManagerAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
